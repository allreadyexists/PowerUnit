...
public readonly struct M_DP_NA_1_Single
{
    ...
    public static void Parse(Span<byte> buffer, ref AsduPacketHeader_2_2 header, DateTime dateTime, IAsduNotification notification)
    {
        for (ushort i = 0; i < header.Count; i++)
        {
            ...
            notification.Notify_M_DP(ref header, value.Address, value.Value, value.Status, dateTime, 0);
        }
    }

    public static int Serialize(byte[] buffer, ref AsduPacketHeader_2_2 header, ref M_DP_NA_1_Single M_DP_NA_1_Single)
    {
...
public readonly struct M_DP_NA_1_Sequence
{
    ...
    public static void Parse(Span<byte> buffer, ref AsduPacketHeader_2_2 header, DateTime dateTime, IAsduNotification notification)
    {
        ...
        for (var i = 0; i < header.Count; i++)
        {
            ...
            notification.Notify_M_DP(ref header, (ushort)(address1.Address + i), value.Value, value.Status, dateTime, 0);
        }
    }

    public static int Serialize(byte[] buffer, ref AsduPacketHeader_2_2 header, ref M_DP_NA_1_Sequence M_DP_NA_1_Sequence)
    {
        throw new NotImplementedException();
    }
}
